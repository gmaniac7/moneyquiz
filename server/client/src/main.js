		...
		controller: function($scope, $rootScope, $http) {
			...
			$scope.uploadFile = function(inputElement) {
				...
				xhr.addEventListener("load", function(res) {
					var url = 'https://admin.shopping-quiz.com/uploads/' + res.currentTarget.responseText;
			...
			$scope.customActions = {
				downloadUsersThatWon: function(obj){
					var win = window.open(`https://admin.shopping-quiz.com/files/won_coupons/${$scope.getValueFromKeyPath(obj, '_id')}`, '_blank');
					if (win) {
				...
				openAddCodesToCoupon: function(obj){
					var win = window.open(`https://admin.shopping-quiz.com/coupon/${$scope.getValueFromKeyPath(obj, '_id')}/couponcodes/massive`, '_blank');
					if (win) {
						//Browser has allowed it to be opened
						win.focus();
					} else {
